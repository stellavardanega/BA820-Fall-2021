...
corpus = ['I want my MTV! www.mtv.com', 
          "Can't I have it all for $5.00 @customerservice #help"]
...
# define a function that extract the score per doc
def sentiment(text):
...
# test the hypothesis of changing sentiment over time
tweets['created'] = pd.to_datetime(tweets.created_at)

# sort the records
tweets.sort_values("created", inplace=True)
...
# spacy naturally wants to operate on a documentå
...
nlp("very great")._.polarity
nlp("not very great")._.polarity
nlp("not a great toy")._.polarity
...
# subjectivity?
nlp("not a great toy")._.subjectivity
nlp("python is the best programming language")._.subjectivity
nlp("python is a free open source programming language")._.subjectivity
